...
function setupCanvas(barChartData) {
    ...
    //     .append('text');
    // header.append('tspan').text('Total revenue by game in $US')
    ...
    const xAxis = d3.axisTop(xScale_v3).tickFormat(formatTicks).tickSizeInner(-chart_height).tickSizeOuter(0);
    const xAxisDraw = this_svg.append('g').attr('class', 'x axis').call(xAxis);

    const yAxis = d3.axisLeft(yScale).tickSize(0); //tickSize()一次設定好
