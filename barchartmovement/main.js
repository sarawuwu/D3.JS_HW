...
function setupCanvas(barChartData, moviesClean) {
    ...
    function update(data) {
        ...
        header.text(`Top 15 ${metric} movies ${metric === 'popularity' ? '' : 'in $US'}`)
    ...
    let header = $("#header").attr('transform', `translate(0,${-chart_margin.top / 2})`).text('Total 15 xxx movies')
    ...
    function mouseover(e) {
        ...
        d3.select('.tip-body').selectAll('p').data(bodyData).join('p').attr('class', 'tip-info').html(d => `${d[0]}:${d[1]}`);
